...
QUnit.test("simpli.isDeepDefined()", function(assert) {
    var foo = {
        bar: {
            baz: [1,2,3,4,5,6]
        }
    };
    assert.strictEqual(simpli.isDeepDefined(foo, 'bar', 'baz'), true, 
        "should return `true` if chain of properties is defined");
    assert.strictEqual(simpli.isDeepDefined(foo, 'apple'), false, 
        "should return `false` if chain of properties is absent");

    var bar = [[1,2], [3,4], [5,6]];
    assert.strictEqual(simpli.isDeepDefined(bar, 0, 1), true, 
        "should return `true` if array index is defined");
    assert.strictEqual(simpli.isDeepDefined(bar, 3, 1), false, 
        "should return `false` if array index is absent");

    assert.strictEqual(simpli.isDeepDefined(foo, 'bar', 'baz', 3), true, 
        "should return `true` if a mixed of properties is defined");
    assert.strictEqual(simpli.isDeepDefined(foo, 'bar', 2, 3), false, 
        "should return `true` if a mixed of properties is absent");
});
